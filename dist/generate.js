 ...
 */
export function generate(src = conf.apiFile, dest = conf.outDir, generateStore = true, unwrapSingleParamMethods = false, swaggerUrlPath = conf.swaggerUrlPath, omitVersion = false) {
    let schema;
    try {
        ...
        if (e instanceof SyntaxError) {
            ...
            out(`JSON scheme file '${src}' does not exist`, TermColors.red);
        out(`${e}`);
        return;
    }
    ...
    processPaths(schema.paths, `http://${schema.host}${swaggerUrlPath}${conf.swaggerFile}`, config, definitions, schema.basePath);
}
